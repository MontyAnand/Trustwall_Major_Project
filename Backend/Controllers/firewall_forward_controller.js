...
module.exports.addForwardRule = (req,res)=>{
    ...
    let command = `sudo nft add rule inet USER_TABLE REDIRECT iifname "${req.body.interface}" `;

    if(req.body.saddrType === 1){ // if saddrType === 0 then saddr is single IP, if saddrType === 1, saddr is a Set
        command = command + `ip saddr @${req.body.saddr} `
    }
    else if(req.body.saddrType === 0){
...
module.exports.updateForwardRule = (req,res)=>{
    ...
    const isDeleted = deleteRule('inet','USER_TABLE','REDIRECT',handle);
    if(! isDeleted){
