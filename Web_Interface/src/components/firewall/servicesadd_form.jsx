...
export const Serviceaddform = ({ visibleDialog }) => {

    const showToast = useToast();
    const dispatch = useDispatch();
    const { isLoading, error } = useSelector(state => state.service);
    ...
    const editServiceOnClick = async () => {
        try {
            await dispatch(createService(serviceName));
            ...
            if(error) throw new Error(error);
            visibleDialog(false);
            showToast("success", "Service Saved", "Service details added successfully!");
        } catch(err) {
