...
import { createZone, getDescription, getShort, getTarget, getZones, setDescription, setShort, setTarget } from "../../Actions/firewall/zone_actions";

export const Zoneaddform = ({ visibleDialog }) => {
    ...
    const [zoneShort, setZoneShort] = useState('');
    const [zoneDescription, setZoneDescription] = useState('');
    const [zoneTargetChecked, setZoneTargetChecked] = useState(true);
    const [zoneTarget, setZoneTarget] = useState('default');
    ...
    const editZoneOnClick = async () => {
        try {
            await dispatch(createZone(zoneName));
            if(error) throw new Error(error);
            await dispatch(setShort(zoneName, zoneShort));
            if(error) throw new Error(error);
            await dispatch(setDescription(zoneName, zoneDescription));
            if(error) throw new Error(error);
            await dispatch(setTarget(zoneName, zoneTarget));
            if(error) throw new Error(error);
            await dispatch(getZones());
            if(error) throw new Error(error);
            await dispatch(getShort(zoneName));
            if(error) throw new Error(error);
            await dispatch(getDescription(zoneName));
            if(error) throw new Error(error);
            await dispatch(getTarget(zoneName));
            if(error) throw new Error(error);
            visibleDialog(false);
            showToast("success", "Zone Saved", "Zone details added successfully!");
        } catch (err) {
